    ...
    describe "PATCH /update/:id" do
        ...
        context " params are ok" do
            it " return http status" do
                patch "/api/v1/category/update/#{category1.id}", params: {category: {name: "Eletrodomésticos"}}
                expect(response).to have_http_status(:ok)
            end
        end
        context " params are nil" do
            it " return http status bad_request" do
                patch "/api/v1/category/update/#{category1.id}", params: {category: {name: nil}}
                expect(response).to have_http_status(:bad_request)
            end
        end
        context " params are not unique" do
            it " return http status bad_request" do
                patch "/api/v1/category/update/#{category1.id}", params: {category: {name: category2.name}}
                expect(response).to have_http_status(:bad_request)
            end
        end
    end
